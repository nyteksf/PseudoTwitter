...
$("#visitorProfileInfoDiv").append(
    '<div id="containerDiv"><h4 id="visDisName"><strong>' +
        visDisName +
        "</strong>"
);
$("#visitorProfileInfoDiv").append(
    '<p id="visUName"><small>@' + visUName + "</small></h4></div>"
);
...
$("#visitorProfileInfoDiv").append(
    '<div id="tweetsVisDiv"><p id="tweets">TWEETS</p>'
);
$("#visitorProfileInfoDiv").append(
    '<p id="tweetVisOutput"><small><span id="woot">' +
        tweetVisOutput +
        "</span></small></p></div>"
);
...
let updateTrendDisplayDiv = function () {
    ...
    while (streams.trendingTweets.length > 0) {
        ...
        $trend.html(
            '<h5 class="hashtagItems"><strong id="trendStrongText">' +
                trend[0] +
                "</strong><br><small><big>" +
                trend[1] +
                " " +
...
function startTime() {
    ...
    if (buttonHidden === true || countOnce) {
        ...
        if (lastStreamsHomeLength !== 0 || streams.home.length > 11) {
            curTweetCount = lastStreamsHomeLength
                ? streams.home.length - lastStreamsHomeLength
                : streams.home.length - 11; //0 -- FRACTAL COMPARISON TO OUTER CLOSURE
            ...
            if (
                streams.home.length - lastStreamsHomeLength === 0 ||
                lastStreamsHomeLength == null
            ) {
                $("#newTweets-btn").hide();
            }
            lastStreamsHomeLength > 0
                ? $("#newTweets-btn").val(
                      view +
                          (streams.home.length - lastStreamsHomeLength) +
                          words
                  )
                : $("#newTweets-btn").val(view + curTweetCount + words);
        }
    }

    if (streams.home.length - 11 === 0) {
        $("#newTweets-btn").val("No new Tweets");
    }
...
$(document).ready(function () {
  ...
  */
    ...
    while (index >= 0) {
        ...
        var $tweet = $('<h5 class="tweetsOutput"></h5>');
        $tweet.html(
            '<img class="tweetIcon" src="' +
                tweet.icon +
                '"/><big><small><strong><span class="tweetText">' +
                tweet.disName +
                '</span></strong> <span class="tweetSubtext">@' +
                tweet.user +
                ' &middot; <span data-livestamp="' +
                tweet.created_at +
                '"></span></span><br><span class="tweetText">' +
                tweet.message +
                "</span></strong></big></small>"
        );
        $tweet.append(
...
$("body, #trendingTweetsDiv, #visitorProfileInfoDiv, .navbar").click(function (
    e
) {
    if (e.target !== this) {
...
function setCookie() {
    document.cookie =
        "display-name=" + visDisName + "; dispN-set=true; path=/;";
    //ADD EXPIRY OF 1 MOS
}
...
function checkCookie() {
    ...
    if (user != "") {
        ...
        window.visitor = visDisName =
            prompt(
                "Please enter your desired Display Name unless remaining anonymous:\nDon't worry. You will only be asked once."
            ) || "Visitor Account"; //NEED FILTER TEXT!
